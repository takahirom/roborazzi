...
| [references/junit_platform_reporting.md](references/junit_platform_reporting.md) | Attaching captured images (golden/actual/compare) to the standard Gradle `Test` report via the `roborazzi-junit-platform-reporting` module (JUnit Platform `fileEntryPublished()`), including setup, the `excludeEngines("junit-vintage")` requirement, and setup diagnostics. |
