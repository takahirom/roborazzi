...
class DesktopPreviewRenderProfile private constructor(
  /**
  ...
  override fun equals(other: Any?): Boolean {
    ...
    if (other !is DesktopPreviewRenderProfile) return false
    return defaultDevice == other.defaultDevice
  }

  override fun hashCode(): Int = defaultDevice?.hashCode() ?: 0

  override fun toString(): String = "DesktopPreviewRenderProfile(defaultDevice=$defaultDevice)"
  ...
  companion object {
     ...
     * `@Preview(device = ...)`.
     */
    val Desktop: DesktopPreviewRenderProfile = DesktopPreviewRenderProfile(defaultDevice = null)
     ...
     */
    val Default: DesktopPreviewRenderProfile get() = Desktop

    fun decode(value: String): DesktopPreviewRenderProfile {
      val parts = splitUnescaped(value).filter { it.isNotEmpty() }
      require(parts.firstOrNull() == VERSION) {
        "Malformed DesktopPreviewRenderProfile: expected it to start with '$VERSION', but was '$value'"
      }
      val fields = parts.drop(1).associate { field ->
        val separator = field.indexOf('=')
        require(separator >= 0) { "Malformed DesktopPreviewRenderProfile field: $field" }
        field.substring(0, separator) to field.substring(separator + 1)
      }
      return DesktopPreviewRenderProfile(
 ...
 */
@ExperimentalRoborazziApi
fun roborazziSystemPropertyDesktopRenderProfile(): DesktopPreviewRenderProfile? =
  getSystemProperty(ROBORAZZI_DESKTOP_RENDER_PROFILE_PROPERTY)
    ?.takeIf { it.isNotEmpty() }
    ?.let { DesktopPreviewRenderProfile.decode(it) }

@ExperimentalRoborazziApi
const val ROBORAZZI_DESKTOP_RENDER_PROFILE_PROPERTY: String = "roborazzi.desktop.renderProfile"
