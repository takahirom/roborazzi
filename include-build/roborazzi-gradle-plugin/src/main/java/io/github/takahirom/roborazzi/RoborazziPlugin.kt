...
// Stable, namespaced ids for the three diagnostics, used both as the grep anchor in each
// message and as the token a user lists in roborazzi.suppress. Keep these strings stable.
 ...
 *
 * Two of the three problems are warnings (ids [OldGradleDiagnosticId] and
 * [NotJUnitPlatformDiagnosticId]). The third ([DoubleExecutionDiagnosticId]) — the stock
 * junit-vintage engine running alongside roborazzi-vintage, which double-executes every
 * test — is a build error by default, because no valid configuration wants both engines to
 * run the same tests. Listing an id in [suppressedDiagnostics] (from the
 * [RoborazziDiagnosticSuppression.PROPERTY] Gradle property) silences a warning outright and
 ...
 */
private fun diagnoseJUnitPlatformReporting(test: Test, suppressedDiagnostics: Set<String>) {
  ...
  // 3. On the JUnit Platform, but does the stock junit-vintage engine actually still run
  // the tests alongside roborazzi-vintage? It runs only when it is not excluded AND not
  // filtered out by a non-empty includeEngines list. A config that keeps it out of the
  // execution set — excludeEngines("junit-vintage") OR includeEngines(...) without
  // junit-vintage (e.g. includeEngines("roborazzi-vintage")) — is correct and not flagged.
  val vintageExcluded = JUnitVintageEngineId in options.excludeEngines
  val includeEngines = options.includeEngines
  val vintageAllowedByIncludeFilter =
    includeEngines.isEmpty() || JUnitVintageEngineId in includeEngines
  val vintageActuallyRuns = !vintageExcluded && vintageAllowedByIncludeFilter
  if (vintageActuallyRuns) {
    // A build error by default: there is no valid setup where both the stock engine and
    // roborazzi-vintage should run the same tests, so this is a 100% misconfiguration.
    // Listing the id in the suppress property downgrades it to a warning.
    val downgraded =
