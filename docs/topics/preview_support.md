...
- `widthDp`/`heightDp`: the preview is wrapped in a fixed-size box. For a preview without a `device` under the `Desktop` profile, 1dp equals 1px at scale 1; `renderScale` reads that density as 160dpi and scales it to a whole dpi, the same way it scales a device's dpi. Otherwise they are dp at the device density, after that scaling. When neither is specified the preview still renders wrap-content.
