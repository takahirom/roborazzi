...
annotation class RoboComposePreviewOptions(
   ...
   * file size at the cost of fidelity. Only the rendering shrinks, not the per-preview setup, so
   * scaling down a large preview is worth far more than scaling down a small component, which is
   * what makes this per-preview override useful. See the `renderScale` property of the Gradle
   * extension `generateComposePreviewRobolectricTests` for what the scale changes and what it
   * does not.
   *
